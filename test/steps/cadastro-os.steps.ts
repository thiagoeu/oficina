...
defineFeature(feature, (test) => {
  ...
  // Cenário 1: Criar ordem de serviço completa com peça e serviço válidos
  ...
  // Cenário 6 : Criar OS apenas com peça (sem serviço)
  test('Criar OS apenas com Peça (sem serviço)', ({
    ...
    when(
      ...
      async (path, table) => {
        ...
        response = await request(server).post(path).send(payload);
        console.log('Cenário 6: Criar OS apenas com Peça (sem serviço)');
      },
    );
  ...
  // Cenário 7: Criar OS com peça fora de estoque
  test('Criar OS com peça fora de estoque', ({ given, and, when, then }) => {
    ...
    when(
      ...
      async (path, table) => {
        ...
        response = await request(server).post(path).send(payload);
        console.log('Cenário 7: Criar OS com peça fora de estoque');
      },
    );
  ...
  // Cenário 9:  Criar OS com valor de pagamento inconsistente (valor negativo ou zero)
  test('Criar OS com valor de pagamento inconsistente (valor negativo ou zero)', ({
    given,
    and,
    when,
    then,
